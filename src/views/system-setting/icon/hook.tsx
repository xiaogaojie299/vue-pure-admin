...
import { getKeyList, is } from "@pureadmin/utils";

import { formRules } from "./rule";
import type { FormItemProps } from "./types";

import {iconTypeMap} from "../constants"
export function useIcon(tableRef: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      prop: "name",
      minWidth: 100,
      formatter: ({ type }) => iconTypeMap[type]?.showLabel
    },
    {
      label: "排序",
      prop: "sortNum",
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getIconList(toRaw({ ...filterParams, ...pagination, pageNum: pagination.currentPage })).finally(() => {
      loading.value = false;
    });
