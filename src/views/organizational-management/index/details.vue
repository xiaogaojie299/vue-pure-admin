...
import SelectOrgTagDialog from "./components/ScoreDetails.vue";
...
const initGetOrgDetail = async () => {
  ...
  let dataFileRequests = responseData.data?.dataFileRequests.filter(
    v => !!v.fieldName
  );
...
<template>
  <div>
    ...
    <template v-if="currentType === '评分指标'">
      <el-scrollbar height="75vh">
        <SelectOrgTagDialog :orgId="id"></SelectOrgTagDialog>
      </el-scrollbar>
    </template>
    <div v-show="currentType === '组织信息'">
        <!-- 页面内容 -->
        <ScrollAnchorNav
          :anchors="groupList"
          content-height="82vh"
                ...
                >
                  ...
                  >
                    <template v-if="info.fieldJson?.label?.length">
                      <el-space>
                        <div
                          v-for="(label, index) in info.fieldJson.label"
                          :key="index"
                        >
                          {{
                            ["服务区域"].includes(info.fieldName)
                              ? label?.join("-")
                              : label
                          }}
                        </div>
                    ...
                    <template v-else>
                      <template
                        v-if="['营业执照', '公司logo'].includes(info.fieldName)"
                      >
                        <el-image
                          fit="cover"
                          preview-teleported
    ...
    <div class="text-center mt-5">
      <el-button type="" round class="w-[180px]!" @click="router.go(-1)"
        >返回</el-button
      >
      <el-button type="primary" round class="w-[180px]!" @click="goEditOrg"
        >编辑</el-button
      >
    </div>
  </div>
